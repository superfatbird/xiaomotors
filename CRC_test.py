...
Packet = [0xFF, 0xFF, 0xFD, 0x00, 0x01, 0x07, 0x00,	0x55, 0x00,	0x06, 0x04,	0x26] # CRC_L will be 0x65(DEC101), CRC_H will be 0x5D(DEC93) 
# Packet = [0xFF, 0xFF, 0xFD, 0x00, 0x01, 0x03, 0x00, 0x01] # CRC_L will be 0x19(DEC25), CRC_H will be 0x4E(DEC78)
# print(', '.join(hex(x) for x in crc_table))

# get the size of the packet without CRC bits
packet_size = len(Packet)
for data in Packet:
    i = ((crc_accum >> 8) ^ data) & 0xFF
    crc_accum = (crc_accum << 8) ^ crc_table[i]
print (crc_accum & 0xFF)
print (crc_accum >> 8 & 0xFF)

# update_crc(crc_accum, packet, packet_size)